...
One such example is fast micro-payments.
...
\footnote{%
  ...
  to ensure a random leader election.
}
...
through an unpredictable number of anonymous and untrusted server nodes, trying to guarantee
data consistency.
...
The amount of taxes depends on two factors: 1) the number
of transactions in a particular wallet and 2) the age of the wallet.
Section~\ref{sec:taxes} provides more details.

\textbf{No Trust}.
The network of communicating nodes maintains the wallets of Zold users.
...
One ZLD by convention equals $2^{32}$ \emph{zents} (4,294,967,296).
...
Corrupted data must be detected early and filtered out as quickly as possible.
...
its nodes to invest their CPU power into meaningless and repetitive
...
The host name and the IP are used in the prefix in order to prevent a Sybil attack,
...
The node tries different suffixes until one of them produces
...
And so on. All hashes must have the same number of trailing zeroes to make
the score valid.
...
is the number of trailing zeros in the hash. In the example above the
...
Similar to the Bitcoin network, which, according to~\textcite{hayes2017},
``automatically adjusts the difficulty
variable so that one block of bitcoins is found, on average, every ten minutes,''
the Zold network may use the strength in order to calibrate itself.
...
Transactions with a positive amount don't have signatures.
...
transfer up to $2^{63}$ zents, which, by convention, is $2^{32}$ ZLD.
...
A one-year-old wallet with 4096 transactions will pay approximately 16 ZLD in taxes annually
...
The most active remote node will be selected as the tax receiver.
...
The following comparison demonstrates the cost of processing a thousand transactions
in different payment systems, including Zold. We have included both well-established systems and relatively new ones.
...
The comparison demonstrates that Zold is one of the most cost-effective cryptocurrency payment systems on the market.
...
the list contains a limited number of pre-defined addresses%
...
\begin{enumerate}
  ...
  \item Availability: the number of errors seen recently;
  \item Knowledgeability: the number of nodes this node is aware of;
...
\dd{X-Zold-Score} HTTP header. The header, if it exists, may include
...
auto-discovery mechanism nodes become aware of each other's presence
in the network.
...
nodes by the number of other remote nodes it promotes. The longer the list a node
...
number of push requests they send. Thus, in order to stay on top of these rankings
...
This Section summarizes the most important of those threats and explains
...
transactions to other wallets. Both owners of these money-receiving
wallets will think that their money arrived, while only one of them is
a legitimate receiver; the other transaction is fraudulent.
...
The proposed architecture has two obvious advantages over many other
...
them. It is possible to make a large number of payments in one batch and
