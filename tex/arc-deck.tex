...
\slide{%
  ...
  \topic{zblue}{SQS Queue}{%
    It is a large global cross-project queue of claims, maintained
    in the AWS SQS. Anyone can drop
    a claim into the queue; it is a very fast operation. The processing
    of the claims happens later, when the dispatcher is up to it.}
  \topic{zblue}{XML}{%
    Each claim is a short XML document with a pre-defined
    structure, validated against an XSD Schema. It includes information
...
\slide{%
  ...
  \begin{multicols}{2}
    \topic{zblue}{Webhooks}{%
      ...
      which send events in JSON format via a RESTful API, which Zerocrat
      provides for them. For higher stability, the \href{http://www.rehttp.net}{ReHTTP}
      web service is used
    ...
    \topic{zblue}{Decoupling}{%
      The way chat hubs are decoupled from the Farm through the queue of claims
      ...
      validating operations, no matter how complex the request is. The
    ...
    \topic{zblue}{Modular Architecture}{%
      ...
      be integrated with a few dozen of them. Even though the complexity of each of
      ...
      simplifies this task and makes the system more extendable.}
  \end{multicols}
  \end{multicols}
}
...
\slide{%
  ...
  \begin{multicols}{2}
    ...
    \topic{zblue}{Brigade}{%
      ...
      the more complex the Policy is, the bigger the number
      of stakeholders.}
    \topic{zblue}{Configuration}{%
      ...
      Policy, the behavior of stakeholders changes immediately.}
    \topic{zblue}{XML}{%
